...
func TestListFile(t *testing.T) {
	c := supa_storage.NewClient(rawUrl, token, map[string]string{})
	resp, err := c.ListFiles("shield", "", supa_storage.FileSearchOptions{
		Limit:  10,
		Offset: 0,
		SortByOptions: supa_storage.SortBy{
			Column: "",
			Order:  "",
		},
	})
...
func TestCreateUploadSignedUrl(t *testing.T) {
	c := supa_storage.NewClient(rawUrl, token, map[string]string{"apiKey": token})
